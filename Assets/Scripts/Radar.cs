...
//	public Texture2D radarTexture;
	...
	public float radarSize = 0.25f;  // The amount of the screen the radar will use
	...
	void Start ()
	{
		...
		if (radarType != RadarTypes.Textured)
		{
			...
			CreateRoundTexture((Texture2D)radarTexture, radarBackgroundA, radarBackgroundB);
		}
		...
		enemy = GameObject.FindGameObjectsWithTag(radarBlip1Tag); 
	}
	...
	void OnGUI ()
	{
		...
		if (radarType != RadarTypes.Transparent)
		{
			...
			{
				Rect frameRect = new Rect(_radarCenter.x - _radarWidth / 2 - 5f, _radarCenter.y - _radarHeight / 2 - 5f, _radarWidth + 10f, _radarHeight + 10f);
...
//		
		...
		{
			Rect centerRect = new Rect(_radarCenter.x - 2f, _radarCenter.y - 2f, 4, 4);
...
//			float dist = Vector3.Distance(centerPos, extPos);
			...
			if(Mathf.Abs(bX) <= _radarWidth*0.5f && Mathf.Abs(bY) <= _radarHeight*0.5f)
			{
				Rect clipRect = new Rect(_radarCenter.x - bX - 2, _radarCenter.y + bY - 2, 4, 4);
				GUI.DrawTexture(clipRect, blipTexture);
			}
//			// For a round radar, make sure we are within the circle
//			if(dist <= (_radarWidth) * 0.5 / radarZoom)
//			{
//				Rect clipRect = new Rect(_radarCenter.x - bX - 2, _radarCenter.y + bY - 2, 4, 4);
//				GUI.DrawTexture(clipRect, blipTexture);
//			}
